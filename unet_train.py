...
with tf.Session(config=config) as sess:
    ...
    for step in range(1, 160001):
        ...
        loss_tr, l2_loss_tr, predicts_tr, _ = sess.run(
            ...
            feed_dict={
                ...
                lr: learning_rate})
        if step * args.batch_size % 5000 == 0:
            train_text = 'step: {}, loss_tr: {}'.format(
                step, loss_tr)
            print(train_text)
        if step == 50000 or step == 100000:
    ...
    saver.save(sess, model_path, write_meta_graph=True, global_step=160000)