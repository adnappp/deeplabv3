...
class Pspnet():
    ...
    def forward_pass(self, x):
        ...
        # 
        with tf.variable_scope('psp', reuse=tf.AUTO_REUSE):
            print(res[2].get_shape())
            x  = self.PyramidPoolingModule(res[2])

            x_size  = x.get_shape().as_list()
            x  = tf.image.resize_bilinear(x, (x_size[1]*2,x_size[2]*2))
            x  = self._conv(x, 1, 256, strides=1, scope='up3_1', batch_norm=False)
            x  = self._conv(x, 3, 256, strides=1, scope='up3_2', batch_norm=False)
            x  = tf.image.resize_bilinear(x, (x_size[1]*4,x_size[2]*4))
            x  = self._conv(x, 1, 128, strides=1, scope='up2_1', batch_norm=False)
            x  = self._conv(x, 3, 128, strides=1, scope='up2_2', batch_norm=False)
            x  = tf.image.resize_bilinear(x, (x_size[1]*8,x_size[2]*8))
            x  = self._conv(x, 1, 64, strides=1, scope='up1_1', batch_norm=False)
            x  = self._conv(x, 3, 64, strides=1, scope='up1_2', batch_norm=False)
            x  = tf.image.resize_bilinear(x, (x_size[1]*16,x_size[2]*16))
            x  = self._conv(x, 1, 64, strides=1, scope='up_1', batch_norm=False)
            x  = self._conv(x, 3, 64, strides=1, scope='up_2', batch_norm=False)
            x  = self._conv(x, 1, 4, 1, 'logits', False, False)
            print(x.get_shape())
            return x
    def _A_ASPP(self):
        pass
    def PyramidPoolingModule(self, x):
        """
        """
        with tf.variable_scope('pspnet'):
            ...
            interp_block1 = self._max_pool(x, 1, 1, 'max')
            interp_block1 = self._conv(interp_block1, kernel_size=1, filters=512, strides=1, scope='pool1',batch_norm=True, activation=True)
            interp_block1 = tf.image.resize_bilinear(interp_block1, (feature_map_size[1],feature_map_size[2]))
            
            interp_block2 = self._max_pool(x, 2, 2, 'max')
            interp_block2 = self._conv(interp_block1, kernel_size=1, filters=512, strides=1, scope='pool2',batch_norm=True, activation=True)
            interp_block2 = tf.image.resize_bilinear(interp_block2, (feature_map_size[1],feature_map_size[2]))
            
            interp_block3 = self._max_pool(x, 3, 3, 'max')
            interp_block3 = self._conv(interp_block1, kernel_size=1, filters=512, strides=1, scope='pool3',batch_norm=True, activation=True)
            interp_block3 = tf.image.resize_bilinear(interp_block3, (feature_map_size[1],feature_map_size[2]))
            
            interp_block6 = self._max_pool(x, 6, 6, 'max')
            interp_block6 = self._conv(interp_block1, kernel_size=1, filters=512, strides=1, scope='pool6',batch_norm=True, activation=True)
            interp_block6 = tf.image.resize_bilinear(interp_block6, (feature_map_size[1],feature_map_size[2]))

            
            net = tf.concat([x, interp_block6, interp_block3, interp_block2, interp_block1], axis=-1)
            net = self._conv(net, kernel_size=1, filters=512, strides=1, scope='net_bn',batch_norm=True, activation=True)
            net = self._conv(net, kernel_size=1, filters=512, strides=1, scope='net',batch_norm=True, activation=True)
